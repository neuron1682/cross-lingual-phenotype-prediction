...
class DefaultFlowCallback(TrainerCallback):
    ...
    def on_step_end(self, args: TrainingArguments, state: TrainerState, control: TrainerControl, **kwargs):
        ...
        # Evaluate
        if args.evaluation_strategy.value == "steps" and state.global_step % args.eval_steps == 0:
    ...
    def on_epoch_end(self, args: TrainingArguments, state: TrainerState, control: TrainerControl, **kwargs):
        logger.info("At Epoch End-Default CallBack")
        logger.info(f"Evaluation Strategy:{args.evaluation_strategy.value}")
        if args.evaluation_strategy.value == "epoch":
            logger.info("Turning Should Evaluate=TRUE")
            control.should_evaluate = True
            if args.load_best_model_at_end:
                control.should_save = True
        return control
...
class EarlyStoppingCallback(TrainerCallback):
    ...
    def __init__(self, greater_is_better, early_stopping_patience: int = 1,
    ...
    def check_metric_value(self, args, state, control, metric_value):
        ...
        logger.info("******EARLY STOPPING----Inside Check Metric Function*****************")
    ...
    def on_train_begin(self, args, state, control, **kwargs):
        ...
        logger.info("******EARLY STOPPING----Inside On Train Begin Function*****************")
    ...
    def on_evaluate(self, args, state, control, metrics, **kwargs):
        ...
        metric_loss = metrics.get("eval_loss")
        metric_pr_auc = metrics.get("eval_val_pr_auc")
        tune.report(eval_val_auc=metric_value, eval_val_loss=metric_loss, eval_val_pr_auc=metric_pr_auc)
        logger.info("******EARLY STOPPING----Inside Evaluate Function*****************")
        logger.info(f"roc_auc={metric_value},loss={metric_loss},training_iteration={state.global_step}")
